...
\begin{frame}	\frametitle{Who are we?}

Me: Robin Donatello - Biostatistician by training

\vspace{1cm}

My amazing student assistants: 
\begin{itemize}
  \item Aldine Ruble - Statistics Major, Econ Minor
  \item Jongwoo Choi- Statistics and General Math Major
  \item Ricardo Aguilera - Statistics Major, Data Science Minor (unofficial)
\end{itemize}
...
Few loaner laptops available -- please share with a neighbor.
...
\begin{itemize}
  \item Pre-configured laptops passed around
